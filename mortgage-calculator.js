 ...
 * Version: 3.1 - State Name Dropdown Fix
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Data object for state tax rates (abbreviation-based)
    ...
    // NEW: Data object mapping state abbreviations to full names
    const STATE_NAMES = {
    ...
    const GlobalVoiceControl = { /* Full voice control logic is maintained */ };

    const MortgageCalculator = {
        init() {
            this.bindEvents();
            this.populateStates(); // This function is now corrected
        ...
        populateStates() {
            ...
            Object.keys(STATE_NAMES).forEach(abbr => {
                if (STATE_TAX_RATES[abbr]) {
                    // Use the full name for the text and the abbreviation for the value
                    const option = new Option(STATE_NAMES[abbr], abbr);
                    stateSelect.add(option);
                }
            });
        
            stateSelect.value = 'CA'; // Set default selection to California
        },
    ...
    const AmortizationTable = {
        render(calculation) {
            ...
            tbody.innerHTML = '';
            calculation.schedule.slice(0, 12).forEach(p => {
